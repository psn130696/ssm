...
public class LogAop {
    ...
    public void doBefor(JoinPoint jp) throws NoSuchMethodException {
        visitTime = new Date();
        claszz = jp.getTarget().getClass();      //具体访问的类
        //获取执行的方法名称
        ...
        if (args==null || args.length==0){
            method = claszz.getMethod(methodName);//无参的方法
        }else {
    ...
    public void doAfter(){
        ...
        if (claszz!=null && method!=null && claszz!=LogAop.class){
            ...
            if (claszzAnnotation != null){
                String[] classValue = claszzAnnotation.value();//获取到类上requestmapping上的val值
                ...
                sysLog.setExecutionTime(time);//执行时常
                sysLog.setMethod("[类名]"+claszz.getName()+"[方法名]"+method.getName());//访问方法名
                sysLog.setUsername(userName);//访问当前用户
